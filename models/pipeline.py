...
def get_best_pipeline(numeric_column: list[str], categorical_column: list[str], X_train: pd.DataFrame, y_train: pd.Series, model_name: str):
    random_state = 42
    models = {
        'LogisticRegression': {
    ...
    if models.get(model_name) == None:
        raise ValueError(f'Неверное имя модели. Доступные названия моделей: {list(models.keys())}') 
    ...
    print('Реализуем автоматический подбор гиперпараметров с помощью RandomizedSearchCV')
    ...
    model_log = {
        ...
        'metrics': {
            ...
            "best_precision": random_search.cv_results_['mean_test_precision'][best_idx],
